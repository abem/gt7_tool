...
- **背景**: 采方針指示。GT7テレメトリの`road_plane_x/y/z`（路面法線ベクトル）は既にデコード済み・配信済みだったが、CAR ATTITUDEカードには一切渡っていなかった。車体`rotation_pitch/roll`（サス圧縮の影響を受ける車体姿勢）とは異なる量であるため、路面そのものの勾配として明確に区別して表示する。
- **実装**: `car-3d.js`に`setRoadSlope(nx, ny, nz, vx, vz)`を新設。法線ベクトルの水平成分を進行方向（`velocity_x/z`）へ射影し、縦断勾配（GRADE, %）とカント（CANT, °）を分離算出。低速時（進行方向が不安定、2m/s未満）は方向非依存の全体傾斜角（TILT, °）へフォールバック。`road_plane_*`欠損時（旧スキーマ録画のREVIEW/リプレイ再生等）は`0%`へ丸めず`--`表示にする。既存の`drawReadout()`（PITCH/ROLL/YAW数値表示）に行を追加する形で統合し、車体姿勢とは別色（アンバー系）・区切り付きで視覚的に分離。`websocket.js`の`updateTyreState()`内、既存の路面法線デバッグ表示直後に配線を追加（既存の呼び出し・表示は無変更）。ライブ受信経路（`telemetry.py`/`decoder.py`/`main.py`受信ループ）は無改変・読み取り専用。
- **検証**: `git diff`で受信経路無改変・既存DOM id無削除を確認（各exit 0）。実測データ（`gt7data/2026-07-18_02_55_05_CAR-485_Lap-10.json`, 6140フレーム）で法線ベクトルが単位ベクトルであること、全体傾斜角が0.05°〜6.25°の妥当なレンジであること、車体姿勢との相関が弱いこと（区別表示の妥当性）を確認。TEST MODEでのPlaywright実機検証で、通常時（GRADE/CANT表示）・`road_plane_*`欠損時（`--`表示）・低速時（TILT表示）の3状態を数値・スクリーンショット双方で確認、`pageerror`0件。

---

## 2026-07-18 — 過去ラップのCSVインポート機能（#177/#178）

### feat: 自前CSVからのラップ取込に対応
- **背景**: #174/#175で実装したCSVエクスポート機能の対応機能。#177で往復インポートの実現可否調査（自前CSVの往復のみ対象、他ソフト形式は対象外と結論）、#178で調査報告書§5のMVPスコープをそのまま仕様として実装（別途の仕様書ゲートは省略）。
- **前提（必須明記）**: 対象は本ツールが出力したCSV（`format=csv`）の往復インポートのみ。MoTeC/RaceChrono/iRacing等の他ソフトウェア形式は、実ファイルでの検証手段が無いため対象外。
- **実装**: `main.py`に`POST /api/laps/import`（multipart/form-data）を新設。CSV→JSON逆変換（`_csv_row_to_sample`）は、エクスポート側の`_csv_columns`/`_csv_row`の列展開規則（`tyre_temp`等の4輪配列⇔`_fl`/`_fr`/`_rl`/`_rr`、`flags`⇔`flag_*`、`course`⇔`course_*`）を正確に反転させて実装し、`CSV_WHEEL_FIELDS`/`CSV_FLAG_KEYS`/`CSV_COURSE_KEYS`の各定数を逆引きにそのまま再利用。取込データは実記録データ（`gt7data/`）と物理的に完全分離した`gt7data_imported/`へ、既存v2スキーマ（JSONサンプル配列）で保存。ファイル名はアップロードされたファイル名を一切使わず、サンプル自身の`timestamp`/`car_id`/`lap_count`から導出し既存の`LAP_FILE_RE`で自己検証（パストラバーサル対策）。ヘッダ列名不一致・`timestamp`/`car_id`欠落・値の型変換失敗はファイル全体を400で拒否（部分取込は行わない）。アップロードサイズ上限100MB（超過時413）。`_scan_lap_files`/`/api/laps`を`log_dir`/`source`引数で拡張し、`include_imported=true`のオプトイン指定時のみ`gt7data_imported/`も合わせて一覧に含める（既定は`gt7data/`のみで無回帰）。`review-view.js`のREVIEW一覧に「インポート済みを表示」チェックボックスと「CSVインポート」ボタンを動的注入し、インポート済み行には**IMPORTED**バッジを表示。ライブ受信経路（`telemetry.py`/`decoder.py`/WebSocket）には一切触れない設計。
- **検証**: 実測最大級ラップ（59,546フレーム、全69フィールド）でのCSV往復変換が全サンプル・全フィールドで完全一致することを確認。データ検証拒否8パターン（未知列/timestamp欠落/car_id欠落/値欠落2種/型エラー/データ行なし/空ファイル）全て意図どおり拒否・最小正常系は受理を確認。実HTTPエンドポイントでの往復（エクスポート→アップロード）・オプトイン表示・詳細取得フォールバック・再CSVエクスポート・既定動作（`include_imported`未指定時の除外）を確認。アップロードサイズ上限超過（413）・パストラバーサル狙いのfilename（実際の保存先・ファイル名には一切影響しないことを確認）・不正CSV（400、`gt7data_imported/`への書込みなし）のセキュリティ系検証を実施。`gt7data/`側ファイルのmd5不変（無変更）を確認。既存回帰スイート（carddrag_smoke・cardgroups_smoke）全PASS。Playwright実機検証でインポートUIのチェックボックス/ボタン注入・バッジ表示切替・ファイル選択アップロード完了メッセージ・エラーなしを確認。検証中、アップロード完了メッセージが直後の一覧再読込で即座に上書きされ視認不能だったUXの不具合を発見し、`setTimeout`で一覧再読込を1.2秒遅延させて修正。
...
- **背景**: #162の検証中、作(saku)が800px幅では`.dashboard`の横スクロールが残存する(#replay-bar起因分は解消済み)ことを発見。`race-metrics.css`内の`#rm-replay-card`(`#rm-replay-grid`内の`#rm-gg-replay`/`#rm-susp-replay`が各固定260px、折返し・縮小対応なし)が`#replay-bar`と同型の構造欠陥を持つことを分離特定(強制非表示での再測定で単独寄与を確認)。査(sa)が独立検証済み。
- **実装**: `race-metrics.css`の`#rm-replay-card`へ`min-width: 0`、`#rm-replay-grid`へ`flex-wrap: wrap`を追加（+9行）。card-drag.js・card-groups.js・`.dashboard`のグリッド定義・replay.css（#162是正済み）は無変更。
- **検証**: 800pxで`#rm-replay-card`実測幅776px。#162是正済みコードとの統合状態で`.dashboard`横スクロール解消（scrollWidth=clientWidth=800）・EXIT機能維持を確認。1920×1080・1066px（#136安全幅）で回帰なし。#153（CARD GROUPS）・#155（再生バードラッグ）に回帰なし。本番実機（main 51da381）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-18 — replay-barビューポート横スクロール是正（#162）

### fix: 画面幅950px以下で#replay-barが.dashboardの横スクロールを引き起こす問題を解消
- **背景**: 査(sa)が#155のコード水準査読中、ビューポート幅を段階的に狭めてジオメトリ検証していて発見（#155のスコープ外の副次発見）。原因はCSS Gridの既定`min-width: auto`により`#replay-bar`内部要素（ボタン群・セレクト・スクラバー）が折り返さない一列レイアウトのまま最小コンテンツ幅（実測約1084px）を下回れないこと。`width: min(100%, 880px)`は視覚上の希望幅指定のみで、1000px未満では縮小効果が出ない。#155由来の新規回帰ではなく既存特性であることをA/B比較で確認済み（既知の安全幅1066pxでは非発現）。
- **実装**: `replay.css`の`body.replay-mode #replay-bar`へ`min-width: 0`（グリッドアイテムの自動最小幅フロア解除）+`flex-wrap: wrap`（内部要素の折返し許容）を追加（+7行）。`.dashboard`のグリッド定義自体・card-drag.jsのコア関数は無変更。
- **検証**: 950px・1066px（#136既知安全幅）・1920×1080で実測。950pxでの横スクロール解消・EXIT機能維持、1066px・1920×1080で回帰なしを確認。800px時点の残存超過は#replay-bar起因ではないと分離特定し、別チケット（#165）へ切り出し。#155実装済みのドラッグ・リサイズ機能に回帰なし。本番実機（main ee42f36）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-18 — 再生バードラッグ対応とサイズ縮小（#155）

### feat: 再生バー（#replay-bar）をドラッグ移動・サイズ変更可能にし、既定表示幅を縮小
- **背景**: ユーザー要望2点（采指示）: 再生バーをドラッグで移動可能にする、バー自体の幅を縮小する。
- **実装**: `index.html`（class付与2箇所。`#replay-bar`へ`card`/`replay-bar-card`、`#replay-title`へ`card-title`併記しcard-drag.jsのBLOCK_SEL・ownTitle()に適合）、`card-drag.js`（`minSize()`へ再生バー専用分岐1件`{w:480,h:32}`。自然高約39pxに対応し#148是正（#150）と同型の位置ズレ再発を防止）、`replay.css`（`width: min(100%, 880px)` + `margin-inline: auto`で要素幅のみ縮小・中央寄せ、grid-column:1/-1維持で#136意図保全。card-title併記によるスタイル汚染の中和も追加）。
- **検証**: 実バックエンドでcarddrag_smoke 22/22完全一致、computedStyleで`grid-column:1/-1`・`sticky`・880px中央寄せを直接確認（#136意図保全）。623px縮小状態での全7再生操作（pause/step前後/速度/シーク/play/EXIT）PASS（#135全操作経路教訓）。モード横断+リロード込みで位置誤差0px。副次発見（950px以下viewportでの横スクロール・floatCard()祖先依存非表示リーク）は#155由来の新規回帰ではないと確認のうえ、それぞれ別チケット（#162・#157）へ切り出し。本番実機（main 3acc5b8）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-18 — CARD GROUPS堅牢性改善（#153）

### fix: カード表示グループ管理の保存失敗検知・タブ間同期・操作性を改善
- **背景**: ユーザーからCARD GROUPS全非表示（SHOW ALLでも復帰しない）の報告。査(sa)の調査で、SHOW ALL自体は正常動作するが直接再現できない一方、`cgSave()`の保存失敗握りつぶし（通知なし）により保存失敗時は見た目上復帰してもリロードで全非表示に逆戻りする経路を発見。原因は操作手順（タップ空振り）か保存失敗のいずれか確定困難だが、査発見の保存失敗握りつぶしは再現性のある実在欠陥のため恒久対処を采が承認。
- **実装**: `card-groups.js`/`card-groups.css`のみ（+49/+34行）。①`cgSave()`の保存失敗を検知し画面右下にトースト通知（`role="alert"`・5秒自動撤去・セッション初回のみ、表示は反映済みだがリロードで戻る旨を明示）、②SHOW ALL押下後にパネル自動クローズ+CARDSボタンへの視覚フラッシュ、③`storage`イベント購読によるタブ間同期（ポーリングなし）、④`touch-action: manipulation`付与（ボタン/行/チェックボックス/SHOW ALL）によるタップ取りこぼし低減。既存のグループ管理ロジック（cgApply/cgLoad/cgVisible）・`.cg-hidden`カスケードは無変更。
- **検証**: cardgroups_smoke.py 17/17 PASS（既存12項目維持+新規5項目: パネル自動クローズ・タブ間同期双方向・保存失敗模擬でのトースト出現+表示反映継続・touch-action計算値）。視覚回帰7/7 PASS（通常時の見た目・レイアウト不変）。carddrag_smoke 22/22等の既存回帰スイートにも影響なし。本番実機（main 478bef7）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-18 — floatCard()祖先依存非表示パターン是正（#157）

### fix: モード切替時に浮遊カードが画面上に残り下層UIのクリックを遮る問題を解消
- **背景**: 作(saku)が#155実装中、スモークテストの実行が不安定化したことから副次的に発見。`card-drag.js`の`floatCard()`は浮遊時に要素を`document.body`直下へ再親化するため、祖先要素の`display:none`によるモード別表示制御が外れる。具体例: REVIEW画面で`#rm-review-card`を浮遊させた状態で他モードへ切り替えても画面上に残り続け、下層UIへのクリックを遮蔽していた。査(sa)がALIGN遮蔽・EXIT遮蔽のワーストケースを独自に検証・特定。
- **実装**: `styles.css`（+4行、REVIEW版汎用ルール1件）・`race-metrics.css`（+19行、`#rm-review-card`のid直接ルール+STRATEGY用REVIEW版ルール）。構造変更なし・JS・コア関数は無変更。
- **検証**: float_leak_smoke.py独立再現8/8 PASS+査独自の別要素3種（GEAR RATIOS/LAP HISTORY/CAR ATTITUDE）でリーク0件。査自身が発見したALIGN遮蔽・EXIT遮蔽の再現手順を是正後コードで再実行し解消を実クリックで確認。作の自己発見リーク（STRATEGY）是正も査独自の再現で確認。視覚回帰なし。本番実機（main 5136fc4）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-17 — カード表示グループ管理（#149/#151）

### feat: カードを機能体系ごとにグループ化し、表示/非表示を一括切替可能に
- **背景**: カード数増加による画面煩雑化への対策（改修計画書§4「ANALYSIS画面の情報密度再編」の前倒し着手）。
- **実装**: `card-groups.js`/`card-groups.css`を新規追加、`index.html`へ`data-cardgroup`属性25件+CARDSボタン/パネルDOMを追加。6グループ（CHARTS / PEDALS・DRIVETRAIN / FUEL・STRATEGY / CAR BEHAVIOR / LAP・TIMING / CAR INFO）単位でチェックボックスによるshow/hideをlocalStorageへ永続化。非表示は全モードで優先（`!important`）。REVIEW/再生のRACE METRICSセクションは対象外（将来編入可能な設計を維持）。
- **検証**: コード水準（`card-drag.js`無変更をgit diffで確認）・実機水準（表示/非表示19通り全PASS・2重リロード永続性PASS・ドラッグ位置保持誤差0px・視覚証跡3幅×6グループ=18通り全PASS・#135教訓の操作経路網羅8経路全PASS・pageerrors 0件）とも合格。本番実機（main 06f3f37）へデプロイ済み。

---

## 2026-07-17 — race-metricsカード3件のドラッグ・リサイズ対象外是正+カードキー安定化（#148/#150）

### fix: race-metrics新カード3件（REVIEW/REPLAY/STRATEGY）がドラッグ&リサイズ対象外だった問題を解消
- **背景**: race-metrics機能（P1〜P3）を内包する3つのトップレベルコンテナ（`#rm-review-card`/`#rm-replay-card`/`#rm-strategy-card`）がcard-drag.jsのBLOCK_SEL（`.card, .chart-wrapper, .racing-top-bar`）に必要な`class="card"`を欠いており、ドラッグ・リサイズ操作の対象外になっていた。是正後の実機検証で、STRATEGYカードの復元位置が下端付近で最大24px上方へ押し戻される系統誤差（`minSize()`の一律`{w:140,h:90}`が自然高39pxのバーを強制拡大→`clampTop`許容域縮小）が判明し、追加是正で解消。あわせて、カード追加のたびに保存キー（DOM順序依存）がずれ大規模レイアウトリセットが再発する根本原因（#150）も是正。
- **実装**: `index.html`（3コンテナへ`class="card ...card"`付与+`.card-title`整備）、`card-drag.js`（STRATEGY専用`minSize()`分岐1件`{w:140,h:32}`追加+キー生成をid優先・id無しはslug+同一slug内連番へ改修。コア関数floatCard/persist/restoreSaved等は無変更）。
- **検証**: STRATEGY複数位置（上端/中央/下端）復元スモークで誤差0px。キー安定性5シナリオ全PASS（28キー中変動は設計上許容のエッジケース4件のみ）。carddrag_smoke 16/16 PASS、視覚回帰7/7 PASS。適用時の1回限りのレイアウトリセット（裁可済み）以外は無回帰。本番実機（main c95d5de）へデプロイ済み。

---

## 2026-07-17 — 実レース由来メトリクス6項目の追加（#142〜#147）

### feat: レースエンジニアリング水準の解析指標6項目を追加
- **背景**: 実レース現場（F1・GT・耐久等）のテレメトリ監視項目を体系調査（#142）し、GT7パケットの既存69フィールドから算出可能な項目を優先度つきで提案。采が全6候補の採用を決裁し、P1/P2/P3の3段階（#145/#146/#147、親チケット#144）で実装。全項目とも読み出し系のみでライブ受信経路は無変更。
- **実装**:
  - P1（#145）: G-Gダイアグラム（横G×前後Gの散布図。REVIEW/再生モードのみ、ライブ表示は追加しない=采決裁）、コーナーフェーズ別デルタ（+トレイルブレーキ重なり指標）
  - P2（#146）: サスペンション変位ヒストグラム（REVIEW/再生モード）、タイヤデグ率+ピットウィンドウ表示（ライブ表示にも追加=采決裁。既存カードDOM表示値の1Hz読取方式）
  - P3（#147）: スロットル/ステア滑らかさスコア（ユーザー自身の直近ベスト群を100基準とする相対スコア=采決裁）、energy_recovery/torque_vector表示（非対応車種は生の0.0を表示せず「N/A」の案内文に置き換えて表示）
- **検証**: 各フェーズとも既存グローバルの読み取り専用（書込0件grep）を確認、査読・実機検証合格。P1で確立した検証パターンをP2・P3まで継続。

---

## 2026-07-17 — REVIEW画面のレイアウト崩れ修正（#136）

### fix: A/B比較パネル・SPEED VS DISTANCEチャートの表示崩れを解消
- **背景**: 実機（abem2）でREVIEW画面を確認したユーザーが、右側の比較パネル（A/B選択・Δ・コース情報）の幅が極端に狭く潰れテキストが1文字ずつ縦折り返しで表示され、SPEED VS DISTANCEチャートが完全に空白になる崩れを発見。原因は`#review-root`/`#replay-bar`がgrid配置未指定のため`.dashboard`の3カラムgridの第1カラム（≦272px）へ自動配置されていたこと。P1-3リリース当初から901px以上の全デスクトップ幅で崩壊していたが、視覚回帰のベースライン自体が崩壊状態で撮影され自己比較PASSしていたため検出できていなかった（#135に続く「検証オラクル自体の欠陥」の2例目）。
- **実装**: `review.css`（`body.review-mode #review-root { grid-column: 1 / -1; }`）・`replay.css`（`body.replay-mode #replay-bar { grid-column: 1 / -1; }`、査指摘による同型是正）の各1行。JS・`.dashboard`グリッド定義自体は無変更。再発防止としてvisual_regression_test.pyへジオメトリassert（`min_widths`検査）・複数幅シナリオ（1066px）を追加。
- **検証**: 幅マトリクス再実測（800/900/1066/1400/1920/2560px全て正常。例: 1066px→root:1042/main:706/chart:684、修正前は260/0/0）。再生バー実操作（実データ再生・2幅）でも正常動作・pageerror 0件。視覚回帰7/7 PASS（ベースライン再取得後、ジオメトリassert有効状態での独立比較）・機能テスト10/10維持。本番実機（main 33c3a48）でコード水準査読と完全一致する結果を確認。

---

## 2026-07-16 — 全カード再生モード（#133/#134）

### feat: 過去ラップを全カードで再生するリプレイ機能を追加
- **背景**: REVIEW（距離チャート比較）の上位互換として、過去ラップを走行中と同一の全カード（約20枚）で時間/距離スクラバー再生する（計画書#132・詳細計画書#134、いずれも査読・承認済み）。
- **実装**: `replay-mode.js`／`replay.css` 新規。記録フレームを既存単一入口 `handleTelemetryMessage` へ供給する方式（test-mode.js と同一アーキテクチャ・#128実データハーネスで実証済み）。REVIEW 一覧の▶ボタンから起動。2段ロード（10Hz先行→60Hz背景差替）・倍速0.5〜4x・時間/距離シーク（チャート窓再構築込み）・記録中断ギャップの自動スキップ。
- **安全設計**: サイズ3層ゲート（34MB超=30Hz上限／101MB超=10Hz＋区間選択。全数実測の最大169.7MBを根拠）。ライブ表示との排他は `replayActive` を websocket.js:617（表示遮断）と steer-response.js（**neutralL較正の汚染防止**、#134査指摘反映）の2箇所に併記——**WS受信・サーバ記録は再生中も継続**。終了時は TEST MODE と同じ復元列（resetAnalysis+resetLapState）。再生中はエンジニア通知を抑止（CSS）。v1旧スキーマは再生対象外（REVIEW比較へ自動案内）。
- **検証**: 実データ機能検証15項目 PASS（60Hz差替・シーク再計算一致・neutralL前後比較で不変・排他遷移・多段フォールバック実測）。性能実測: 初回再生可能319ms・60Hz差替1.0s・供給67Hz維持・シーク平均118ms。視覚回帰6/6（再生バーシナリオ追加）・機能テスト10/10維持・既存シンボル無改変（機械確認）。
...
- **背景**: 走行データは `gt7data/` に毎日記録される（実測 1,085 ラップ・7.4GB）一方、読み戻す機能が無く死蔵されていた（GT7改修計画書 原因1）。P1詳細計画書 §2 の設計に基づく実装。
- **バックエンド（main.py）**: 読み出し専用 API 2本を新設。`GET /api/laps`（一覧。ファイル名メタのみで軽量、limit/offset/date/car_id）と `GET /api/laps/{file}`（詳細。fields 射影・every 間引き・既定 60Hz→10Hz）。ワイルドカード静的ルートより前に登録し、パース・直列化は `asyncio.to_thread` でイベントループ（ライブ WS 配信）を塞がない。**受信・記録・WS 配信経路は無変更**。
- **フロントエンド**: `review-view.js`／`review.css` を新規追加（既存ファイルの変更は index.html への要素追加と menu.js のセグメント3ボタン化のみ）。ツールバー `ANALYSIS│DRIVE│REVIEW`。一覧（日付グループ・日付フィルタ）→ A/B 選択 → 距離基準チャート3種（速度重畳・タイムデルタ・スロットル/ブレーキ）＋比較サマリ（近似タイム・Δ・コース名）＋BEST基準。旧スキーマ（2026-02系、lap_count/course 無し）にもフォールバック対応。
- **設計訂正**: 記録データの `current_laptime`（パケット 0x80）は実際には**ゲーム内時刻の進行**でありラップ経過時間ではないことが実データ検証で判明（別途 #127 で診断）。時間軸・タイム近似は受信 `timestamp` のクランプ付き累積（dt<2s のみ加算）に統一した。
- **検証**: 視覚回帰 5/5（REVIEW シナリオを新規追加）・機能テスト 10/10 維持・スモーク（切替/一覧/2世代重畳/REVIEW中のTEST MODE 継続）全 PASS・uncaught pageerror 0件。既存トップレベルシンボルの改名・削除 0件（全ファイル grep 確認）。

---

## 2026-07-16 — current_laptime誤用の是正（#127/#128）

### fix: ライブ側のRUNNING表示・ライブデルタ・推定ラップが実走行時に無意味な値になる不具合を修正
- **背景**: P1-3(#123)の事前検証中に発見しエスカレーション（#127）。`decoder.py`が`current_laptime`として格納する値（パケットオフセット0x80）は、ラップ経過時間ではなく**ゲーム内時刻の進行ms**であることを実データ（定数51,300,000=14.25時間・単調増加58,200,000起点等）で確定。ライブ側3系統（走行中ラップタイム表示・ライブデルタ・推定ラップ）が実走行時に無意味な値になる実害（RUNNING表示が数百時間相当・デルタが常時固定/数千秒相当）を確認（クラッシュなし・静かな誤動作。TEST MODEは正しい値を合成するため検出できなかった）。
- **実装**: 対応方針案1（フロントのみ是正、案2のdecoder正名化・案3の表示撤去は不採用）を采が承認。`telemetry-analysis.js`（時間軸を「受信timestampのクランプ付き累積秒」方式へ変更。P1-3 REVIEW機能で実証済みの方式を踏襲。TEST MODE合成フレームはtimestampを持たないため`current_laptime`差分フォールバックの2経路設計）、`websocket.js`（RUNNING表示をラップ内クロック参照へ変更）。`decoder.py`・`telemetry.py`・バックエンド・記録スキーマは無変更。
- **検証**: 実データ2ケースで前後再現（時刻固定レース: RUNNING 855:00.000/デルタ+0.00s固定→2:35.658/-0.02s（GT7確定2:35.497と0.1%差）。時刻進行×21レース: +8100.00s→+0.05s）。pageerror 0件。TEST MODEフォールバック経路6項目PASS・視覚回帰5/5・機能テスト10/10維持。既存トップレベルシンボルの削除・改名0件（追加は`updateLapClock`/`LAP_CLOCK_GAP_S`の2つのみ、全ファイル衝突grep 0件）。

---

## 2026-07-12 — リサイズ機能の実装後・敵対的レビュー（Opus）と堅牢化修正

### fix: 計画書 §5-10 の実装後レビューを完走し、CONFIRMED 3件を修正
- **背景**: リサイズ機能の実装後レビュー（敵対的・反証者2名/所見）は fable 試用枠切れで未完だった。Opus 4.8 で 19 エージェント（3観点発見 → 各所見を反証者2名で敵対検証）を完走。**所見8件 → CONFIRMED 5 / 棄却 3**、重複排除で実質4件。
- **A. タッチ端末でグリップのリサイズがパンに奪われる**（§7-7 の実装落とし込み漏れ）→ `#gt7-resize-grip` と `body.gt7-resizing` に `touch-action:none` を追加（ドラッグ用 `.card-drag-handle` と同じ流儀）。Pointer Events では `preventDefault` ではパンを抑止できず `touch-action` CSS が必須。
- **B. ウィンドウ縮小/レイアウト跨ぎでグリップが画面外に取り残されリサイズ不能**（非スクロール=デスクトップ。垂直クランプが水平と非対称。**反証0/2＝最強所見**）→ `clampTop` の非スクロール分岐を「上端40pxのみ可視」から「カード全体＝下端グリップまで画面内」（`innerHeight − h`）に変更し、水平の `pageW()−w` クランプと対称化。
- **C. DRIVE で display:none 化したカードにグリップが残り、不可視カードをリサイズして {0,0,0,0} を保存**→ `startResize` 冒頭で `getClientRects().length===0`（非描画）なら `hideGrip` して no-op。
- **据置（設計どおり）**: DRIVE モードの `min-height` が floating の縮小高を上書きする件（反証1/2、うち1名は「意図挙動」判定）→ DRIVE は走行中にヒーロー要素を意図的に拡大する仕様（計画書 §3.3）。データ破損なし・ANALYSIS 復帰で元サイズに戻るため挙動据置、計画書 §8 に明記。
- **検証**: headless 8項目合格（TEST MODE 駆動 / touch-action=none / グリップ・リサイズ浮遊+拡大+永続化 / 縮小後もグリップ画面内 / 非表示カードで壊れ値を保存しない / 未捕捉例外0）。
...
- **報告**: 「CAR ATTITUDE のヨーだけ角度の数字がやたらでかい」。
- **診断**: 実走 20,417 フレームで `(rotation_pitch, rotation_yaw, rotation_roll, orientation)` の二乗和が**常に 1.0000** — GT7 パケット 0x1C-0x28 は**単位クォータニオン (x,y,z,w)** であり、旧実装はその成分をラジアンのオイラー角として誤読していた。
  - YAW: 成分（±1）を度数化した無意味な値。さらに TEST MODE のデモは `t*0.05` で**無限増加**（数百°に到達 — 報告の直接原因）。
  - PITCH/ROLL: 小角度では成分≈半角のため「実際の約半分」が表示され、もっともらしく見えて発覚しなかった。
- **修正**: decoder.py でクォータニオン→真のオイラー角(rad)に変換して送出（フィールド名不変・フロント無改変で全消費者が正しくなる）。符号規約は実データで検証:
  - yaw = 世界ヘディング ±π（速度ベクトル方位と**中央値 0.18°** で一致、12万フレーム）
  - pitch 正=機首上げ（`corr(sin(pitch), vy/v) = +0.97`）
  - roll 正=右ロール（右輪サス圧縮差と正相関、17万フレーム）
- **HDG の統一**: 方角表示も旧「acos(orientation)+速度符号ハック」を廃し `rotation_yaw` 直接使用に。CAR ATTITUDE の YAW と同一規約で自己整合し、**後退中の東西反転という旧制約も解消**。
- **TEST MODE**: デモの回転もオイラー角セマンティクスに整合（pitch±5°/roll±8°、yaw=軌跡の進行方位、orientation=cos(θ/2)）。
- **検証**: TEST MODE 実測で yaw が周回に沿って ±180° 内を巡回し HDG と完全一致（+39.8°↔40°NE / −59°↔301°NW）、pytest 12 passed、未捕捉例外0。

---

## 2026-07-11 — 表示内容の仮説駆動改善: TEST MODE 経路統一・偽装機能撤去・表示の意味修正

### 手法: 仮説→実測検証→実装（全 fable、仮説20件中 CONFIRMED 17件を実装）
4次元（ツールバー操作/空状態/一瞥可読性/視覚一貫性）で問題仮説を生成し、各仮説を headless 実測とコード実証で厳格判定。実装後の敵対的レビュー（3観点×反証者2名、31エージェント）で追加 CONFIRMED 5件も即修正。

### refactor: TEST MODE をライブと同一の処理経路へ統一（最大の構造改善）
- **症状**: TEST MODE 10秒でも読み値30項目が '--'/'Waiting for data...' のまま（ラップ/BEST/履歴・ギア比・タイヤ rps/半径・路面・座標・HDG・車両ID・グリッド順位等）。BEST が空なのに PB トーストが出る自己矛盾も。
- **原因**: renderDemoFrame が個別 DOM を手作業で叩き、正準更新関数群を一切通らない実装だったため。
- **修正**: decoder.py のキー名に忠実な**完全合成パケットを組み立てて `handleTelemetryMessage` を1回呼ぶ**方式へ全面改修。単位変換（clutch /100・susp /1000・加速度 m/s² 統一）、STEER RESPONSE のみ誇張舵角のデモ専用上書き。**TEST MODE がライブ表示経路そのものの検証になる**という副次価値。
- **停止時リセット**: `resetLapState()` 新設 — デモの偽 BEST/履歴/最高速/グリッド順位がライブ復帰後に残留する回帰を防止。
- **ライブ遮断ガード**: 実走テレメトリ受信中に TEST MODE を点けても 60Hz ライブと 5Hz デモが DOM を奪い合わない（WS 取り込み側で一時停止、停止で即復帰 — **実機データ受信中に実証済み**）。

### refactor: 偽装機能の撤去 — セクタータイム S1/S2/S3
- GT7 テレメトリパケットに**セクター情報は存在せず**（decoder.py/main.py に供給ゼロ）、実走では永久に '--.---'。TEST MODE だけが乱数で偽値を生成し「動く機能」に見せていた。表示行・更新関数・偽値生成・CSS を全撤去し、ドキュメントに非対応の根拠を明記。

### fix: 表示の意味修正（実測で確認された誤解誘発の解消）
- **HDG**: orientation の生値 '0.734' → **度数+8方位 '133° SE'**（acos 変換+velocity で東西解決。後退中の反転は既知の制約として文書化）。HGT に m 単位。BODY ACCEL タイトルに (m/s²)。
- **燃料 EST LAPS**: 立ち上がり（1周完了前）の「赤い '--'」→ 推定確定まで中立色 '--'、確定後は 0.0 も数値表示（main.py のセンチネル 0 を正しく解釈。DRIVE 側の同型バグも修正）。
- **デルタ3状態化**: 基準ラップ未成立/±0.05s は中立グレー、速い=緑/遅い=赤のみ明示（「ベストが無いのに緑」の誤メッセージ解消）。ラップ履歴の色クラスも統一。
- **ギア色**: 1-2速で常時アンバー（偽警告）を廃止、レブリミット付近（rpm≥96%）のみ変色。
- **接続ピル段階表示**: 'Connected (no data)' → 初回テレメトリで 'Connected' → 'TEST MODE'（inline style を廃し CSS クラス化。「接続してるのに壊れてる」誤認の解消）。
- **通知トースト位置**: ヘッダー実高に追随（ツールバー右端ボタンへの被り解消）。
- **--text-muted コントラスト**: 3.8:1 → 5.2:1（9-11px の小サイズラベル全面）。
- **ツールバー**: ラベルを ALIGN / FULLSCREEN に統一（バー内の言語体系一貫化）、矢印キーでのフォーカス巡回（ロービング tabindex）、DRIVE 中のタッチ標的 44px 化。
- **検証**: 空値30項目→全充足を実測、停止時リセット→実機ライブデータの正常引き継ぎを実 PS5 受信中に確認、全段階で未捕捉例外0。docs 5本を実装と一字一致まで同期。

...
1. ✅ **`showConnectionError` の二重定義**（`fix/duplicate-definitions-2026-07-09` で**修正済み**）— `ui_components.js` の定義（フローティングトースト）が `websocket.js`（後読み込み）で上書きシャドウされ死コード化していた。実行中は websocket.js 版（ステータスピル更新）が有効なため、死んでいた ui_components.js 版を削除（挙動保存）。
2. ✅ **`getSectorClass` の二重定義**（`fix/duplicate-definitions-2026-07-09` で**修正済み**）— `websocket.js`（ガード付き・閾値0.1/0.3）が実テレメトリ用の正準版だが、`test-mode.js`（ガード無し・閾値0.3/0.6）が後読み込みで**実テレメトリ側の呼び出しまで上書きシャドウ**していた（ベスト未設定時に `''` でなく `'red'` を返す不具合を含む）。test-mode.js 側の重複を削除し、両経路をガード付き正準版に一本化。詳細は [`docs/bugfix-duplicate-definitions-2026-07-09.md`](bugfix-duplicate-definitions-2026-07-09.md)。

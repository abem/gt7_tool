...
> ※ TEST MODE トグルには誤操作防止の連打防止（250ms）があります。素早く2回クリックしても「開始→即停止」にはならず、1回分のみ反応します。
...
1. **ヘッダーの操作ツールバーを使用**
   - ツールバー左端の **ANALYSIS│DRIVE** セグメントで、切り替えたい側をクリック（点灯している側が現在のビュー。点灯側のクリックは無効）
   - DRIVE 表示中もツールバーは残るため、**ANALYSIS** をクリックすれば1クリックで戻れる

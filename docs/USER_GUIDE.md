...
- **車体姿勢 (CAR ATTITUDE)**: Canvas2D でサスペンション・ステア連動タイヤ・重心点 CoG（荷重移動）、路面勾配（GRADE %／CANT °、車体姿勢とは別枠表示。低速時は全体傾斜角 TILT ° へ切替、路面データ欠損時は `--`）を可視化
...
6. **CSVダウンロード**: 一覧の各行にある **⬇ CSV ボタン**を押すと、そのラップの記録データをCSV形式でダウンロードできます。速度・ペダル入力・タイヤ温度・姿勢角など記録済みの全項目が含まれます。ダウンロードしたファイルは表計算ソフト（Excel等）や外部の分析ツールで開けます。**自前形式の単純な出力であり、MoTeC・RaceChrono・iRacing等の他ソフトウェアの形式との互換性は未検証・未保証です。**
   - **上級者向け（FastF1互換エクスポート、#434 P2）**: ダウンロードUIはありませんが、URLに`?format=fastf1`を付けて`/api/laps/{file}?format=fastf1`へアクセスすると、Python [FastF1](https://github.com/theOehrly/Fast-F1)ライブラリの列規約（`Speed`/`RPM`/`nGear`等）に合わせたCSVを取得できます。単一車両の記録のため、タイヤ交換・他車位置等の情報は含まれません（詳細は[API.md](API.md#fastf1互換エクスポートformatfastf1)参照）。
7. **CSVインポート**（#177/#178）: 左ペイン上部の **CSVインポート ボタン**から、本ツールが出力したCSVファイル（上記のCSVダウンロードで得られる形式）を取り込めます。取り込んだラップは実記録データ（`gt7data/`）とは別の専用領域（`gt7data_imported/`）に保存され、既定では一覧に表示されません。**インポート済みを表示** チェックボックスをONにすると、**IMPORTED** バッジ付きで一覧に混在表示され、通常のラップと同様にA/B選択・チャート比較・全カード再生・CSV再ダウンロードができます。
   - 取込対象は本ツール自身が出力したCSVのみです。他ソフトウェア（MoTeC/RaceChrono/iRacing等）のエクスポートファイルは形式が異なるため取り込めません。
   - ヘッダ列名の不一致・`timestamp`/`car_id`列の欠落・値の型不正があるファイルは、明確なエラーメッセージとともにアップロード全体が拒否されます（部分的な取込は行いません）。
   - アップロードサイズの上限は100MBです。
...
| **ラップタイム予測（PRED）** | ANALYSIS（ライブ） | 走行中のラップの最終タイムをAI（機械学習モデル）で予測し、STRATEGYカードに表示します。**学習データが十分で予測精度が確認できたコース×車種の組み合わせのみ**表示され、それ以外は`--`のまま（未確定・非対応の中立表示）です。表示時は予測タイムの下に、検証誤差（MAE%）と学習に使ったラップ数（n）を小さく併記し、予測の信頼度が分かるようにしています |
...
| **アウトライヤー検出自動化（マシントラブル警告）** | ANALYSIS（ライブ） | 走行中、①ラップタイムの異常な悪化 ②タイヤ温度の急激な上昇 ③油圧の異常な低下 ④燃料消費率の急増、のいずれかを検知すると、既存のレースエンジニア通知パネルへ自動で警告表示します。④は誤検知（ドライビングスタイル変化との判別困難）が起こりやすいため、緩めの閾値で控えめに通知します |
| **AIハイライト自動生成** | 全カード再生 | 再生データの読み込み時に、Gフォース（横+前後合成）が急変した地点を自動検出し、再生バーのスクラバー上に▲マークで表示します。マーカーをクリックすると該当地点へ即座にジャンプできます。複数ラップを含む記録では、最速ラップとの区間タイム差が急変する地点も別の色（青系▲）で表示されます（単一ラップの記録ではこの指標は表示されません） |

> これらの指標は GT7 パケットの既存フィールドから算出した読み出し専用の表示です。ライブ受信・記録の経路には影響しません。

### バーチャルピットウォール（エンジニア役からの指示）

別端末（スマートフォン・タブレット・PC等）をエンジニア役として使い、ドライバー側の画面へ「FUEL MAP 3」「LIFT & COAST」等の指示を送れます。

1. エンジニア役の端末で `https://<サーバーのアドレス>:8080/engineer` を開きます（メインダッシュボードとは別の専用ページです）。
2. 接続状態が **Connected** になったら、**クイック送信**のプリセットボタン（FUEL MAP 3 / LIFT & COAST）または**自由入力**欄にメッセージを入力して送信できます。送信履歴は画面下部に一覧表示されます。
3. ドライバー側のダッシュボードでは、既存のレースエンジニア通知パネル（画面上部）にメッセージが表示され、同時にブラウザの音声読み上げ機能（追加インストール不要）で読み上げられます。
4. 送信できる相手は同一ネットワーク上の全接続クライアントです。**本機能に認証・アクセス制御はありません**。同一ネットワーク内での利用を前提としています。
5. **ドライバーからの応答**: DRIVE ビューの画面下部に「OK / COPY / RE-PLAN」の3つのタップボタンが表示されます（ANALYSIS ビューでは非表示）。ドライバーがいずれかをタップすると、エンジニア役の画面の「送信履歴」とは別の「ドライバーからの応答」欄に表示されます。走行中でも片手で押せるよう、大きめのタップ領域にしています。

### 音声コマンド（ハンズフリーのビュー/カード切替）

ツールバーの **VOICE** ボタンで、DRIVE/ANALYSIS ビューの切替やカード表示グループ（CARDS の g1〜g6）の切替を音声操作できます。

1. VOICE ボタンを押すと、その瞬間から数秒だけ聞き取りを開始します（ボタンが点滅している間のみ）。**常時待ち受けはしません**。押し忘れた場合や無音のまま数秒経過した場合は自動的に聞き取りを終了します。
2. 認識できるコマンド例:
   - 「ドライブ」→ DRIVE ビューへ切替 / 「アナリシス」または「解析」→ ANALYSIS ビューへ切替
   - 「チャート」→ CHARTS グループのみ表示 / 「ペダル」→ PEDALS/DRIVETRAIN のみ表示 / 「フューエル」→ FUEL/STRATEGY のみ表示 / 「ビヘイビア」→ CAR BEHAVIOR のみ表示 / 「ラップ」→ LAP/TIMING のみ表示 / 「車両情報」→ CAR INFO のみ表示
3. 認識できなかった場合や音声認識エラー時は、既存の通知パネルに簡潔なメッセージが表示されます。
4. **対応ブラウザは Chrome / Edge / Safari のみ**です（`SpeechRecognition` Web API非対応の Firefox 等では、VOICE ボタン自体が表示されません）。また、Chrome/Edge はオンラインでの音声認識のため、インターネット接続が必要です。

### 仮想セクタータイム（S1/S2/S3）

ラップ表示カード（現在ラップ・LAST/BEST/EST）の下に、S1/S2/S3の3区間タイムが表示されます。

- **「仮想」の意味**: GT7のテレメトリパケットには実際のコースセクター情報が含まれていません。そのため本機能は、同じコース・車種での過去ラップ（参照ラップ）の走行距離を**機械的に3等分**した仮想の区切りで区間タイムを算出します。**GT7公式のコースセクター境界とは異なります**のでご注意ください。
- 参照ラップが取得できるまで（走行データの蓄積が必要な最初の走行時等）は`--.---`のまま表示されます。
- そのセッション中の自己ベスト区間タイムを更新すると、該当する区間が紫色でハイライトされます。
- 参照ラップとの走行ラインの違いにより、区間の区切り位置に多少の誤差が生じることがあります。また、参照距離の見積もり誤差により、まれに最後の区間（S3）が確定しないままラップが終わることがあります。

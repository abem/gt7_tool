...
- **レイアウト操作**: 全ブロックをドラッグで自由配置、ヘッダーの操作ツールバー（ANALYSIS│DRIVE 切替 / TEST MODE / 整列 / 全画面）
...
左側に状態を持つコントロール（ビュー切替・TEST MODE）、右側に受動的なユーティリティ（整列・全画面）が空間的に分離して並びます。

| コントロール | 見た目 | 操作 | 状態表示 | ツールチップ文言 |
|-------------|--------|------|----------|------------------|
| **ビュー切替** | `ANALYSIS│DRIVE` の2連セグメント | 切り替えたい側をクリック。点灯している側（現在のビュー）のクリックは無効 | 現在のビュー側が点灯 | ANALYSIS: 「解析用の全パネル表示」<br>DRIVE: 「走行用の最小表示(ギア・シフトライト・デルタ・燃料)」 |
...
| **整列** | `⊞ 整列` ボタン | クリックで全ブロックを初期配置に整列。実行時はボタンが一瞬点灯して応答を返す | 保持する状態なし（即時実行） | 「全ブロックを初期配置に整列 / ブロックはドラッグで移動、ダブルクリックで個別リセット」 |
| **全画面** | `⛶ 全画面` ボタン | クリックで全画面表示と通常表示を切替（`Esc` でも解除可能） | 全画面中は点灯 | 通常時: 「全画面表示に切替」<br>全画面中: 「全画面を解除 (Esc)」 |
...
- 画面幅が **700px 以下**になると、整列・全画面ボタンはアイコン（⊞ / ⛶）のみの表示になります。状態を担う `ANALYSIS│DRIVE` と `TEST MODE` のラベルは常に表示されます
- 横幅に収まらない場合、ツールバーは2行に折り返します

#### ブロック移動のヒントについて

旧メニューの「配置」にあった操作ヒント（ブロックはドラッグで移動・ダブルクリックで個別リセット）は、**整列ボタンのツールチップに集約**されました。整列ボタンにマウスを重ねると確認できます。
...
- **全体リセット**: ツールバーの **⊞ 整列** ボタンで、すべて元の配置に戻ります
...
3. デモデータの表示が開始されます（ピルが点灯し、● ドットが点滅します）
4. もう一度 **● TEST MODE** をクリックすると停止します

### デモ操舵とデモ軌跡

STEER RESPONSE を駆動するデモ操舵と、解析ロジック検証用の内部デモ軌跡が含まれます。仕様の詳細は [docs/test-mode.md](test-mode.md) を参照してください。

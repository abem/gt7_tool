...
- **HDG（方角）**: 度数と8方位の併記で表示されます（例: `133° SE`）。GT7 パケットの方角値は東回り/西回りを区別できないため、速度ベクトルの東西成分から向きを解決しています。このため**後退中は東西が反転して表示されることがあります**（前進を前提とした制約）。速度ベクトルが取得できない場合は北からの偏角の度数のみ表示されます
...
| **ALIGN** | `⊞ ALIGN` ボタン | クリックで全ブロックを初期配置に整列。実行時はボタンが一瞬点灯して応答を返す | 保持する状態なし（即時実行） | 「全ブロックを初期配置に整列 / ブロックはドラッグで移動、ダブルクリックで個別リセット」 |
...
旧メニューの「配置」にあった操作ヒント（ブロックはドラッグで移動・ダブルクリックで個別リセット）は、**ALIGN ボタンのツールチップに集約**されました。ALIGN ボタンにマウスを重ねると確認できます。
...
- **全体リセット**: ツールバーの **⊞ ALIGN** ボタンで、すべて元の配置に戻ります

...
A: 既定では自動削除されません。保存ポリシー（ローテーション）の仕組みが `scripts/gt7data_rotate.py` として用意されており、`config.json` の `data_retention` で有効化すると古いラップを整理できます（既定は無効・実行してもまず表示のみの安全設計）。設定項目・削除の仕組み・保護方法の詳細は [README の「記録データの保存ポリシー」](../README.md#記録データの保存ポリシーローテーション)を参照してください。

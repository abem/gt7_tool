...
GT7 Toolは、グランツーリスモ7 (GT7) のテレメトリーデータをリアルタイムで表示・分析するためのツールです。PS5/PS4から送信されるデータを受信し、ウェブブラウザ上で美しいダッシュボードとして表示します。

### 主な機能

- **リアルタイムテレメトリー表示**: 速度、回転数、ギア、ペダル入力
...
| `receive_port` | テレメトリー受信ポート | 33740 |
...
GT7のテレメトリーパケットにはコースIDが含まれていないため、車両の位置座標 (X, Z) からコースを推定します。
...
1. GT7で学習させたいコースを走行
2. ツールが自動的にテレメトリーデータを `gt7data/` ディレクトリに保存
...
3. GT7で「テレメトリーデータ送信」がONになっているか確認
...
A: テストモードであれば、インターネット接続なしで動作します。ただし、PS5からのテレメトリー受信にはLAN接続が必要です。
...
A: 実際のテレメトリーデータを収集して、`test_course_detection.py` でコースデータベースを更新してください。

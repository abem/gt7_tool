...
GT7 Toolは、GT7（グランツーリスモ7）からのテレメトリーデータを受信・解析・配信するためのAPIを提供します。
...
**説明:** テレメトリーデータのリアルタイムストリームを受信します
...
| `receive()` | テレメトリーパケットを受信 (bytes or None) |
| `close()` | ソケットを閉じる |
...
パケット定義ファイルは、テレメトリーデータのオフセットとデータ型を定義します。
...
GT7のテレメトリーパケットはSalsa20で暗号化されています。

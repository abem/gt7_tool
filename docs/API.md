...
| `/api/laps/{file}` | GET | 単一ラップの詳細（fields射影・every間引き対応） |
...
| パラメータ | 型 | 既定値 | 説明 |
|-----------|-----|--------|------|
| `fields` | string（カンマ区切り） | 既定フィールド集合 | 返却するサンプルのフィールドを絞り込み |
| `every` | int | 実装既定値 | Nフレームごとに1件間引き |

**レスポンス**: `samples`（射影・間引き済みサンプル配列）、`samples_total`/`samples_returned`（元の総件数/返却件数）、`schema`（`v1`/`v2`。`lap_count` の有無で判定）、`course`（コース情報、旧形式データでは省略）等のメタ情報。

存在しないファイル・命名規則不一致は404、破損ファイルは500を返します。

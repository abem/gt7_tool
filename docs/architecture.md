...
| `menu.js` | 操作メニューバー | ヘッダーの Windows 風ドロップダウン（表示/モード/配置/設定）。既存ボタン（TEST MODE / ANALYSIS / コースライン / 配置リセット）を「隠しプロキシ」として `.click()` で駆動し、状態を ✓ 反映 |
...
1. ブラウザでデータ受信
   └─> JSONデータをパース
       ...
       └─> コースマップ・CAR ATTITUDE（Canvas2D）を更新
```
